<x-dashboard.layout title="Claims" breadcrumb="Claims" :user-name="auth()->user()->name" :user-email="auth()->user()->email" :logout-url="Route::has('logout') ? route('logout') : url('/logout')">
    <div class="space-y-6">
        ...
        <form method="POST" action="{{ route('admin.claims.store') }}" class="grid gap-3 border border-[var(--dash-border)] bg-[var(--dash-card)] p-4 md:grid-cols-5">
            ...
            <select name="value_type" class="border border-[var(--dash-border)] bg-[var(--dash-card)] px-3 py-2 text-xs"><option>string</option><option>boolean</option><option>array</option><option>json</option></select>
            <select name="sensitivity" class="border border-[var(--dash-border)] bg-[var(--dash-card)] px-3 py-2 text-xs"><option>public</option><option>personal</option><option>sensitive</option></select>
        ...
        <x-table.wrapper :striped="true">
            <x-slot:head><x-table.th>Key</x-table.th><x-table.th>Source</x-table.th><x-table.th>Type</x-table.th><x-table.th>Sensitivity</x-table.th><x-table.th>Status</x-table.th></x-slot:head>
